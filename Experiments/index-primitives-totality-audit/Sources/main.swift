...
// Status: SUPERSEDED 2026-04-30 — Integer-position type usage no longer valid in this position; experiment relies on a syntactic shape no longer accepted
...
func demonstrateOptionalArithmetic() {
    ...
    // Index + Offset → Index? (ALREADY TOTAL)
    ...
    // Index - Index → Offset (ALWAYS TOTAL)
...
func printSynthesis() {
    ...
    │    - Index + Offset → Index?: Always total, returns nil             │
    │    - Index - Index → Offset: Always total                           │
    │                                                                     │
...
func demonstrateConversions() {
    ...
    print("Int(\(raw)) → Index<Int>(\(idx.position.rawValue))")

    // Index<T> → Index<T>.Bounded<N>
    let bounded: Index<Int>.Bounded<10>? = idx.bounded()
    print("Index<Int> → Index<Int>.Bounded<10>? = \(bounded != nil ? "Some" : "None")")

    // Index<T>.Bounded<N> → Index<T>
    if let b = bounded {
        let unbounded = b.unbounded
        print("Index<Int>.Bounded<10> → Index<Int>(\(unbounded.position.rawValue))")
    }

    // Staying in typed land
    print()
    print("CONVERSION COST ANALYSIS:")
    print("  Int → Index<T>:              1 comparison (check >= 0)")
    print("  Index<T> → Bounded<N>:       2 comparisons (check 0..<N)")
    print("  Bounded<N> → Index<T>:       0 operations (always valid)")
    print("  Index<T> → Int:              0 operations (.position.rawValue)")
    print()
